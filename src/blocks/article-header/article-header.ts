...
const renderTopline = (date?: string) => {
  ...
  let dateIsoString: string | undefined;
  ...
  return html`
    <div class="dva-m-article-header__topline">
      <time class="dva-m-article-header__date">
        <time class="text-dimmed" datetime="${ifDefined(dateIsoString)}">
          ${formattedDate}
        </time>
      </time>
    </div>
...
const renderHeadline = (headline?: string) => {
  ...
  return html`
    <h1 class="dva-m-article-header__headline">${headline}</h1>
  `;
}
...
const renderText = (text?: string) => {
  ...
  return html`
    <p class="dva-m-article-header__text">${text}</p>
  `;
}
