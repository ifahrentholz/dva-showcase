                    ...
                    (img, index) => html`
                      ...
                      >
                        <dva-e-lazy-image
                          ...
                          src="${img}"
                          alt="${args.descriptions ? args.descriptions[index] : ""}"
                          aspect-ratio="${index === 0 ? "16:9" : "1:1"}"
                        ></dva-e-lazy-image>
                        <p class="dvag-m-basic-slider__slide-text" style="width: 535px;">
                          ${args.descriptions ? args.descriptions[index] : ""}
                        </p>
                      </div>
                    `,
                  )
...
export default function (block: HTMLElement) {
  ...
  const aboutMeMediaLinks: { img: string; description: string }[] = [];

  for (let i = 3; i < block.children.length; i++) {
    const imgElement = block.children[i].querySelector("div > picture > img");
    const textElement = block.children[i].querySelector("div:nth-child(2)");
    if (imgElement && textElement) {
      const imgSrc = (imgElement as HTMLImageElement).getAttribute("src") || "";
      const description = textElement.textContent || "";
      aboutMeMediaLinks.push({ img: imgSrc, description });
    }
  }
  ...
  render(
    aboutMeTemplateOverview({
      ...
      headline,
      text,
      images: aboutMeMediaLinks.map(link => link.img),
      descriptions: aboutMeMediaLinks.map(link => link.description),
    }),
    block,
  );
}
